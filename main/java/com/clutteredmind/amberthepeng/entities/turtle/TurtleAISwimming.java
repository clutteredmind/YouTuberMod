...
public class TurtleAISwimming extends EntityAIBase
{
   ...
   public void startExecuting ()
   {
      ...
      BlockPos destBlockPos = new BlockPos (MathHelper.floor_double (randX), MathHelper.floor_double (randY),
            MathHelper.floor_double (randZ));
      Block destBlock = this.entityTurtle.worldObj.getBlockState (destBlockPos).getBlock ();
      Block underDestBlock = this.entityTurtle.worldObj.getBlockState (destBlockPos.offset (EnumFacing.DOWN, 1))
            .getBlock ();

      if (destBlock == Blocks.water
            || (destBlock == Blocks.air && underDestBlock != Blocks.water && underDestBlock != Blocks.air))
      {
